...
interface TableRow {
  [key: string]: any; // Since table rows can have different fields
}
...
const props = defineProps<{
  columns: Column[];
  tableData: TableRow[];
  editRoute: (id: string) => string; // Dynamic route function for edit
  viewRoute: (id: string) => string; // Dynamic route function for view
  deleteRoute: (id: string) => string; // Dynamic route function for delete
}>();
// Emit events for delete confirmation if needed
...
<template>
  <div
    ...
    <div class="max-w-full overflow-x-auto">
      <ButtonDefault route="/users/create" label="+ New User" customClasses="bg-meta-3 text-white rounded-md mb-3" />
      <table class="w-full table-auto">
        <thead>
          <tr class="bg-gray-2 text-left dark:bg-meta-4">
            <th v-for="(column, index) in props.columns" :key="index" class="px-4 py-2 border-b">
              {{ column.label }}
            </th>
        ...
        <tbody>
          <tr v-for="(row, rowIndex) in props.tableData" :key="rowIndex" class="border">
            <td v-for="(column, colIndex) in props.columns" :key="colIndex" class="border px-4 py-2">
              <!-- Correctly access row[column.field] here -->
